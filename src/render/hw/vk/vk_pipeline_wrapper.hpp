...
namespace skity
{
	...
	class AbsPipelineWrapper : public VkInterfaceClient
	{
		public:
			...
			virtual VkDescriptorSetLayout GetFontSetLayout()
			{
	...
	class ComputePipeline : public AbsPipelineWrapper
	{
		...
		protected:
			glm::vec4 const& CommonInfo() const
			{
				return common_info_;
			}
			glm::vec4 const& BoundsInfo() const
			{
				return bounds_info_;
			}

			VKTexture* InputTexture() const
			{
				return input_texture_;
			}
			VKTexture* OutpuTexture() const
			{
				return output_texture_;
			}

			SKVkFrameBufferData* FrameBufferData() const
			{
				return frame_buffer_data_;
			}
	...
	class PipelineFamily : public VkInterfaceClient
	{
		...
		protected:
			...
			bool UseGeometryShader() const
			{
				return use_geometry_shader_;
			}

			HWStencilFunc StencilFunc() const
			{
				return stencil_func_;
			}

			HWStencilOp StencilOp() const
			{
				return stencil_op_;
			}
	...
	class RenderPipelineFamily : public PipelineFamily
	{
		...
		protected:
			...
			VkShaderModule GetGeometryShader()
			{
				return gs_shader_;
			}
